...
packageName = "omegacentauri.mobi.simplestopwatch" if len(argv) < 4 else argv[3]
charsToDump = "0123456789.:\u2212" if len(argv) < 5 else argv[4]
charsToNarrow = ''.join(x for x in ":." if x in charsToDump)
...
public class %s extends MiniFont {
  ...
  public void addFontData() {
    defineFontSize(%gf);
""" % (packageName, className, className, "true" if maximizeDigitBounds else "false", maxY-minY))
